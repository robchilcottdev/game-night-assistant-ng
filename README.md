...
`npm run build` to build the FE and push built FE files to my local HA add-ons folder share via `deploy.js` node script that's triggered in `package.json`'s build script config. And... *breathe*. The backend repo is here: https://github.com/robchilcottdev/game-night-assistant
